...
// The DIPFunc type is an adapter to allow the use of ordinary
// function as DIP mutator.
type DIPFunc func(context.Context, *db.DIPMutation) (db.Value, error)

// Mutate calls f(ctx, m).
func (f DIPFunc) Mutate(ctx context.Context, m db.Mutation) (db.Value, error) {
	if mv, ok := m.(*db.DIPMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *db.DIPMutation", m)
}
